...
<div id="content" style="font-family:Arial, Helvetica, sans-serif; font-size:16px">
  ...
  <p>
   ...
   $terms=trim($_GET["terms"]);
	...
	{
		echo '<script>
		alert("Please make sure the correct input.");
       ...
       <?
	       $sqlcontrol="select count(*) from seed_target_gain_with_base where ".$extra."_id='".$terms."'";
			...
			<? if($row["count(*)"]>0)
			  {
			   ?>
			   <a href="Targets.php?<? echo $extra ?>=<? echo $terms ?>&gainorlost=gain"> Click for detail </a>.
			   <?
			  }
			?>
			</td></tr>
       ...
       <?
	       $sqlcontrol="select count(*) from seed_target_loss_with_base where ".$extra."_id='".$terms."'";
			...
			<? if($row["count(*)"]>0)
			  {
			   ?>
			   <a href="Targets.php?<? echo $extra ?>=<? echo $terms ?>&gainorlost=loss"> Click for detail </a>.
			   <?
			  }
